...
std::shared_ptr<Byte> Hosting::GetBuffer(boost::asio::io_context* context_) noexcept {
    if (!context_) {
        return NULL;
    }

    MutexScope scope_(lockobj_);
    ChunkTable::iterator tail_ = chunks_.begin();
    ChunkTable::iterator endl_ = chunks_.end();
    if (tail_ != endl_) {
        return tail_->second;
    }
    
    std::shared_ptr<Byte> chunk_ = make_shared_alloc<Byte>(UDP_MSS);
    if (!chunk_) {
        return NULL;
    }

    chunks_.insert(std::make_pair(context_, chunk_));
